...
ggplot() +
  ...
  geom_sf(data = selectCentroids %>% filter(TotalPop != 0) %>% st_union(), color = "red", size = 5, fill = NA) +
  facet_wrap(~year) +
  scale_fill_manual(values = palette5,
                    labels = qBr(allTracts.group, "MedHHInc.inf"),
                    name = "Med. HH Income\n(Quintile Breaks)") +
  labs(title = "Med. HH Income", subtitle = "Boston; 2009-2019 (Adjusted for Inflation)") +
  mapTheme() + 
  theme(plot.title = element_text(size=22))

ggplot() +
  geom_sf(data = allTracts.group, aes(fill = q5(pctBachelors))) +
  geom_sf(data = buffer_clipped, color = "red", size = 5, fill = NA) +
  facet_wrap(~year) +
  scale_fill_manual(values = palette5,
